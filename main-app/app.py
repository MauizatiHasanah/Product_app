...
def get_reviews(product_id):
    try:
        ...
        if isinstance(data, dict) and 'data' in data:
            return data['data']  # Pastikan format API konsisten
        elif isinstance(data, list):  # Jika API mengembalikan list
...
def get_product_info(product_id):
    ...
    if request.args.get('format') == 'json':
        return jsonify({
            ...
            "message": "Product data fetched successfully" if combined_response["product"] else "Failed to fetch product data"
        })
